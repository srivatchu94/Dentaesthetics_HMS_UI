...
<body style="margin:0;padding:0;background:#f1f5f9;font-family:Arial,sans-serif;color:#1e293b;">
  ...
  // Normalize GetPatientVisit's response into a sorted array of visit records,
  // regardless of whether the backend returns a single object, an array, or an
  // object wrapping a list (e.g. { visits: [...] }).
  const normalizeVisitHistory = (raw) => {
    let list = [];
    if (Array.isArray(raw)) {
      list = raw;
    } else if (raw && Array.isArray(raw.visits)) {
      list = raw.visits;
    ...
    return [...list].sort((a, b) => {
      const dateA = new Date(a?.visitDate || a?.VisitDate || 0).getTime();
      const dateB = new Date(b?.visitDate || b?.VisitDate || 0).getTime();
      return dateB - dateA;
    });
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log('═══════════════════════════════════════════════════════');
        console.log(JSON.stringify(rawVisitData, null, 2));
        console.log('═══════════════════════════════════════════════════════');

        // Normalize into full history (all visits for this appointment), latest first.
        const visitHistory = normalizeVisitHistory(rawVisitData);
        const existingVisitData = visitHistory[0] || null;
        console.log('📚 Normalized visit history count:', visitHistory.length);
        ...
        if (existingVisitData) {
          ...
          setSelectedAppointmentForVisit({ ...selectedAppointmentDetails, existingVisitData, visitHistory });
        } else {
            ...
            <AnimatePresence mode="wait">
              {!isSidebarCollapsed && (
                ...
                >
                  ...
                  <p className="text-indigo-100 text-xs whitespace-nowrap">Dr.{doctorName || 'Doctor'}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
